...
std::vector<char> decrypt_digit_wires(std::vector<std::string> const e){
	...
	for(short int i = 0, j; i < seven.size(); ++i){
		n_d[0] = seven[i];
	...
	for(short int i = 0, j = 0; i < four.size(); ++i){
    ...
    // Find 0.
	for(short int i = 0, j, k; i < e.size(); ++i){
	...
	for(short int i = 0; i < zero.size(); ++i){
    ...
    // Find 5.
	for(short int i = 0; i < e.size(); ++i){
        if(e[i].length() == 5){                       // It can be 2, 3 or 5.
			for(short int j = 0; j < e[i].length(); ++j){
				if(e[i][j] == n_d[5]) five = e[i];    // Found it!
			}
		}
	}

	for(short int i = 0; i < five.size(); ++i){
	...
	for(short int i = 0; i < one.size(); ++i){
		if(one[i] != n_d[2])
...
int decrypt_digit_display(std::vector<std::string> const& e_d, std::vector<char> const& d_d){
    std::string d = "";
    int n;
    for(short int i = 10; i < 14; ++i){              // 10 digits, 4 digits on display.
...
std::vector<std::vector<std::string>> read_digit_from_file(std::istream& str){
    ...
    if(str.good()){
        ...
        for(short int i = 0; !str.eof(); ++i){
            str.clear();
            getline(str, line);
            iss.clear();
            iss.str(line);
            iss >> digit;
            for(short int j = 0; iss && j < DIG_SIZE; ++j){
...
long long int sum_all_digits(std::vector<std::vector<std::string>> const& e_d){
    long long int sum = 0;
    int n = 0;
